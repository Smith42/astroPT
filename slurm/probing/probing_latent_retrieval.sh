...
PYTHON_SCRIPT="$REPO_ROOT/scripts/probing/probing_latent_retrieval.py"
...
# If EMB_DIR contains .npy files directly, use it. Otherwise, look for subdirectories.
if ls "$EMB_DIR"/*.npy 1> /dev/null 2>&1; then
    DETECTED_EMB=$(readlink -f "$EMB_DIR")
else
    # Try to find the most recent subdirectory (e.g., best_img-mean...)
...
if [ -z "$DETECTED_EMB" ]; then
    echo "[ERROR]: No valid embedding directory found at $EMB_DIR"
    exit 1
fi

# SAVE_DIR defaults to the embedding folder
SAVE_ARG=""
if [ -n "${SAVE_DIR:-}" ]; then
