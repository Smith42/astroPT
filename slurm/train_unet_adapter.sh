...
#SBATCH --job-name=Train_UNet_Adapter
...
#--- LOGS FILES ---#
#SBATCH --output=logs/unet_adapter_%j.out
#SBATCH --error=logs/unet_adapter_%j.err
...
SAVE_DIR="logs/unet_adapter_weights"
RETRAIN_SCRIPT="${REPO_ROOT}/scripts/retrain_unet_adapter.py"

BATCH_SIZE=32
EPOCHS=20
LR=1e-4
TARGET_SIZE=112
TRANSFORM_METHOD="crop"
...
echo "-----------------------------------------------"
echo "Starting U-Net Adapter Training Job $SLURM_JOB_ID - $NOW"
echo "-----------------------------------------------"
...
echo "-----------------------------------------------"
echo "U-Net Adapter Training Finished"
echo "-----------------------------------------------"
