...
def postprocess_image_prediction(
    ...
    # Reorder the predicted channels [VIS, Y, J, H] to match img_gt_raw order [VIS, H, J, Y]
...
def main():
    ...
    img_key = next((k for k in registry.modalities if 'image' in k.lower() or k == 'images'), 'images')
    spec_key = next((k for k in registry.modalities if 'spec' in k.lower() or k == 'spectra'), 'spectra')
        ...
        if device.type == 'cpu':
            for k, v in X.items():
                if isinstance(v, torch.Tensor):
                    is_discrete_mod = False
                    for dk in [img_key, spec_key]:
                        if dk == k:
                            try:
                                mc = registry.get_config(dk)
                                is_discrete_mod = getattr(mc, 'vocab_size', 0) > 0
                            except Exception:
                                pass
