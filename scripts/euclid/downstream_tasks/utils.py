...
def save_and_plot_redshifts(true_redshifts, predicted_redshifts, percentage, modality, parameter, model_name):
    ...
    # Create the directory if it doesn't exist
    os.makedirs(modality, exist_ok=True)
    if len(true_redshifts) == 0 or len(predicted_redshifts) == 0:
        raise ValueError("Input arrays true_redshifts and predicted_redshifts must not be empty.")
    ...
    # Add the y=x line
    max_val = max(np.max(true_redshifts), np.max(predicted_redshifts)) if true_redshifts.size and predicted_redshifts.size else 0
    min_val = min(np.min(true_redshifts), np.min(predicted_redshifts)) if true_redshifts.size and predicted_redshifts.size else 0
    if parameter =="PhotoZ":
