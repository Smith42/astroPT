...
def main():
    ...
    with torch.no_grad(), ctx:
        for batch in tqdm(dl, desc="Extracting AstroCLIP embeddings"):
            ...
            emb_i_norm = F.normalize(emb_i.float(), p=2, dim=-1)
            emb_s_norm = F.normalize(emb_s.float(), p=2, dim=-1)
            emb_j = F.normalize(emb_i_norm + emb_s_norm, p=2, dim=-1)
            
            emb_i_np = emb_i_norm.cpu().numpy()
            emb_s_np = emb_s_norm.cpu().numpy()
            emb_j_np = emb_j.cpu().numpy()
