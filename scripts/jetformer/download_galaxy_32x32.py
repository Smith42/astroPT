...
def main():
    ...
    for ex in ds:
        ...
        if len(buf) == SHARD:
            x = torch.stack(buf, dim=0)  # [N,3,32,32] uint8
            torch.save({"images": x}, os.path.join(OUTDIR, f"shard_{shard_idx:04d}.pt"))
    ...
    print(f"Done. Total saved: {saved} samples into {OUTDIR}/shard_*.pt")
