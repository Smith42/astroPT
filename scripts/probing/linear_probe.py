...
if __name__ == "__main__":
    ...
    if not isfile("zss.npy"):
        ...
        # Check if model uses CLS token for pooling
        reduction = "cls" if getattr(model.config, 'use_cls_token', False) else "mean"
        logger.info(f"Using reduction: {reduction}")

        for B in tqdm(dl):
            emb_dict = model.generate_embeddings(B, reduction=reduction)
            # If reduction was 'cls', the key is 'cls', otherwise it's the modality name
            zs = emb_dict.get("cls", emb_dict.get("images")).detach().numpy()
            zss.append(zs)
