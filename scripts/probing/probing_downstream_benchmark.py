...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--modalities", nargs='+', default=None, help="Filter modalities to evaluate (images, spectra, joint, cls)")
...
def load_data(
    ...
    # Exclude known metadata and ID files
    excluded_files = set(id_candidates) | {'metadata.npy', 'indices.npy'}
...
def filter_catalog(df: pd.DataFrame) -> pd.DataFrame:
    """Applies strict spectral quality filtering (redshift-dependent SNR > 3)."""
    logger.info("Applying Redshift-dependent Spectral Quality Filter (SNR > 3)...")
    initial_len = len(df)
    
    # Spectrograph filter based on redshift Z
    if 'SNR_SPEC_R' in df.columns and 'SNR_SPEC_Z' in df.columns and 'Z' in df.columns:
        mask_snr = ((df['Z'] < 0.15) & (df['SNR_SPEC_R'] > 3.0)) | ((df['Z'] >= 0.15) & (df['SNR_SPEC_Z'] > 3.0))
    elif 'SNR_SPEC_R' in df.columns and 'SNR_SPEC_Z' in df.columns:
        mask_snr = (df['SNR_SPEC_R'] > 3.0) | (df['SNR_SPEC_Z'] > 3.0)
    else:
        logger.warning("SNR_SPEC_R, SNR_SPEC_Z or Z not found. Skipping global SNR filter.")
        mask_snr = pd.Series(True, index=df.index)

    # Combining results
    filtered_df = df[mask_snr].copy()
...
def get_task_data(
    ...
    if target_col not in CLASSIFICATION_TARGETS or pd.api.types.is_numeric_dtype(vals):
        ...
        # Automated dynamic quality/SNR filter using associated IVAR column
        ivar_col = f"{target_col}_IVAR"
        if ivar_col in df.columns:
            ivar_vals = df[ivar_col]
            err_vals = np.where(ivar_vals > 0, 1.0 / np.sqrt(ivar_vals.clip(lower=1e-10)), np.inf)
            mask &= (vals > 3.0 * err_vals)
            logger.info(f"    [Dynamic Filter] Applied SNR > 3 quality filter for regression target '{target_col}' using '{ivar_col}'.")
    else:
...
def train_engine(
...
) -> Tuple[np.ndarray, np.ndarray, int, Optional[np.ndarray], Optional[dict], Optional[Any], int]:
    """
    ...
    if probe_type == 'knn':
        ...
        return preds, y_test_raw, 1, test_ids, None, scaler_y, X_train.shape[1]
    ...
    return np.concatenate(all_preds), y_test_raw, best_epoch, test_ids, model.state_dict(), scaler_y, input_dim
...
def compute_transfer_gap_rows(target_results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for probe in sorted(df_target['Probe'].dropna().unique()):
        ...
        # Identify the two main modalities to compare
        # Prioritize 'EuclidImage' vs 'DESISpectrum' or 'images' vs 'spectra'
        mods = sorted(probe_df['Modality'].unique())
        if len(mods) < 2: continue
        
        m1 = next((m for m in mods if 'image' in m.lower()), mods[0])
        m2 = next((m for m in mods if 'spec' in m.lower() and m != m1), mods[1])
        
        row_m1 = probe_df[probe_df['Modality'] == m1]
        row_m2 = probe_df[probe_df['Modality'] == m2]
        
        if row_m1.empty or row_m2.empty: continue

        row_m1 = row_m1.iloc[0]
        row_m2 = row_m2.iloc[0]
        task = row_m1['Task']

        if task == 'regression' and pd.notna(row_m1.get('R2')) and pd.notna(row_m2.get('R2')):
            primary_metric = 'R2'
            val_m1 = float(row_m1['R2'])
            val_m2 = float(row_m2['R2'])
        elif task == 'classification' and pd.notna(row_m1.get('F1_score')) and pd.notna(row_m2.get('F1_score')):
            primary_metric = 'F1_score'
            val_m1 = float(row_m1['F1_score'])
            val_m2 = float(row_m2['F1_score'])
        else:
            continue

        rows.append({
            'Target': row_m1['Target'],
            'Task': task,
            'Probe': probe,
            ...
            'Value1': val_m1,
            'Value2': val_m2,
            'Gap_1Minus2': val_m1 - val_m2,
            'AbsGap': abs(val_m1 - val_m2),
        })
...
def main():
    ...
    for target_col in final_targets:
        ...
        valid_modalities = sorted([m for m in aligned_embeddings.keys() if m != 'targetid'])
        if args.modalities:
            valid_modalities = [m for m in valid_modalities if m in args.modalities]

        for mode in valid_modalities:
            ...
            for probe in probes_to_run:
                ...
                try:
                    ...
                    for seed in args.seeds:
                        preds, true_vals, best_epoch, test_ids, state_dict, scaler_y, input_dim = train_engine(
                            X,
                            y,
                            probe,
                            task_type,
                        ...
                        score = seed_metrics.get("R2", -float('inf')) if task_type == 'regression' else seed_metrics.get("F1_score", seed_metrics.get("Accuracy", -float('inf')))
                        if score > best_seed_score:
                            best_seed_score = score
                            best_model_data = {
                                "state_dict": {k: v.cpu().clone() for k, v in state_dict.items()} if state_dict is not None else None,
                                "input_dim": input_dim,
                    ...
                    # Store best model weights and scalers in centralized dictionaries
                    if best_model_data is not None and best_model_data["state_dict"] is not None:
                        has_mean = hasattr(best_model_data["scaler_y"], 'mean_') and best_model_data["scaler_y"].mean_ is not None
                        has_scale = hasattr(best_model_data["scaler_y"], 'scale_') and best_model_data["scaler_y"].scale_ is not None
                        has_var = hasattr(best_model_data["scaler_y"], 'var_') and best_model_data["scaler_y"].var_ is not None
                        has_a = hasattr(best_model_data["scaler_y"], 'a') and best_model_data["scaler_y"].a is not None
                        
                        entry = {
                            "model_state_dict": best_model_data["state_dict"],
                            "input_dim": best_model_data["input_dim"],
                            "output_dim": best_model_data["output_dim"],
                            "task_type": best_model_data["task_type"],
                            "score": best_model_data["score"],
                            "metrics": best_model_data["metrics"],
                            "scaler_y_mean": float(best_model_data["scaler_y"].mean_[0]) if (best_model_data["task_type"] == 'regression' and has_mean) else 0.0,
                            "scaler_y_scale": float(best_model_data["scaler_y"].scale_[0]) if (best_model_data["task_type"] == 'regression' and has_scale) else 1.0,
                            "scaler_y_var": float(best_model_data["scaler_y"].var_[0]) if (best_model_data["task_type"] == 'regression' and has_var) else 1.0,
                            "scaler_y_a": float(best_model_data["scaler_y"].a) if (best_model_data["task_type"] == 'regression' and has_a) else 1.0,
                            "scaler_type": "AsinhScaler" if (best_model_data["task_type"] == 'regression' and has_a) else "StandardScaler" if (best_model_data["task_type"] == 'regression' and has_mean) else None
                        }
                        
                        if task_type == 'classification' and processor is not None:
                            entry["classes"] = list(processor.classes_)
                            
                        key = f"{target_col}/{mode}"
                        if probe == 'mlp':
                            mlp_weights_dict[key] = entry
                        elif probe == 'lp':
                            lp_weights_dict[key] = entry
        ...
        if target_results:
            ...
            if gap_rows:
                ...
                gap_cols = [
                    ...
                    'Modality1', 'Modality2', 'Gap_1Minus2', 'AbsGap'
                ]
    ...
    # Save centralized probing MLP and LP weights to disk
    mlp_save_path = save_dir / "probing_MLP.pt"
    lp_save_path = save_dir / "probing_LP.pt"
    
    torch.save(mlp_weights_dict, mlp_save_path)
    logger.info(f" --> Unified MLP Weights saved in: {mlp_save_path}")
    
    torch.save(lp_weights_dict, lp_save_path)
    logger.info(f" --> Unified LP Weights saved in: {lp_save_path}")
